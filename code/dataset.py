...
class TestIVDataset:
    ...
    def get_rest(self, x):
        # Ensure (x - 1) % 12 == 0 so patch_embed (stride=3) and 2x downsample stay even.
        if x <= 1:
            return 0
        mod = (x - 1) % 12
        return 0 if mod == 0 else 12 - mod
    ...
    def _match_size(self, vi, ir, cb, cr):
        if vi.shape[1] == ir.shape[1] and vi.shape[2] == ir.shape[2]:
            return vi, ir, cb, cr
        h = min(vi.shape[1], ir.shape[1])
        w = min(vi.shape[2], ir.shape[2])
        vi = vi[:, :h, :w]
        ir = ir[:, :h, :w]
        cb = cb[:, :h, :w]
        cr = cr[:, :h, :w]
        print("Warning: VI/IR size mismatch, cropped to match.", flush=True)
        return vi, ir, cb, cr
    ...
    def __getitem__(self, index):
        ...
        vi, ir, Cb, Cr = self._match_size(vi, ir, Cb, Cr)
