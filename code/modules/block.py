...
class SS2D(nn.Module):
    def __init__(
        self, d_model, d_state=16, d_conv=3, expand=1.5, dt_rank="auto", dt_min=0.001,
            dt_max=0.1, dt_init="random", dt_scale=1.0, dt_init_floor=1e-4, dropout=0.,
                ...
                gate_type="scalar", dynamic_gate_hidden=32, **kwargs,
    ):
        ...
        # Multi-scale global branch: downsample features, run the same SS2D, then upsample and fuse.
        # Gates are initialized to 0 to keep backward compatibility with released checkpoints.
        ...
        self.dynamic_gate = (
            DynamicScaleGate(self.d_inner, self.global_scales, hidden=dynamic_gate_hidden)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        y_local = y1 + y2 + y3 + y4 # (B, D, L)
        y = torch.transpose(y_local, dim0=1, dim1=2).contiguous().view(B, H, W, -1)

        if self.global_scales:
            # Add coarse global context (learnable gates).
            gates = cssc_gate_values(x, self.global_gates, self.dynamic_gate, H, W)
            for idx, scale in enumerate(self.global_scales):
                Hg = max(1, (H + scale - 1) // scale)
                Wg = max(1, (W + scale - 1) // scale)
                x_g = F.adaptive_avg_pool2d(x, output_size=(Hg, Wg))
                yg1, yg2, yg3, yg4 = self.forward_core(x_g)
                y_g = yg1 + yg2 + yg3 + yg4  # (B, D, Lg)
                y_g = torch.transpose(y_g, 1, 2).contiguous().view(B, Hg, Wg, -1).permute(0, 3, 1, 2)
                y_g = F.interpolate(y_g, size=(H, W), mode="bilinear", align_corners=False).permute(0, 2, 3, 1)
                y = y + gates[:, idx].view(B, 1, 1, 1) * y_g
