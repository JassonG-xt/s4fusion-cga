...
4. **Present study** — CGA mechanism, zero-init compatibility, contributions list; honest boundary: under the single-baseline convention the high-conflict recall gain is met on all three seeds (p=4.88e-04 / 4.34e-03 / 4.34e-03), but under the budget-matched convention the criterion is met on one seed of three (k=1/3; 1/2 effective independent readings), so the gain is not reported as a replicated effect; neither convention reaches the stricter thresholds (+0.02 recall / mAP50 +0.01); and the inference-time content controls show the conflict-triggered restriction (R2) adds nothing at either endpoint.

---

## Assumptions / missing inputs

- **CGA quantitative claim updated (2026-08-25, `eval_str_cga.log`)**: strengthened CGA (32 epochs, w_commit 10 + gate_lr 0.01) on the full 300-image M3FD detection protocol gives high-conflict recall 0.4385 → 0.4579 (**+0.0194**, recovered=20 lost=3, exact binomial/McNemar-style **p=4.88e-04**, significant); recall all-objects +0.0120; mAP50 0.7197 → 0.7282 (+0.0085). **Honest boundary — the stricter gates were NOT met**: gate-C1 (+0.02 high-conflict recall threshold: 0.0194 < 0.02), gate-C2 (mAP50 delta +0.0085 < +0.01), gate-C3 (AG structural non-inferiority fails at exactly −1.00%; SF passes at −0.85%; EN passes). The draft's claims below are scoped to what the numbers support: *significant, direction-consistent gains on the primary endpoint, with the residual gap to the stricter thresholds reported as such* — never "passed the gates." **Three-seed robustness completed 2026-09-12** (`seeds_table.md`): H5 replicates on all three seeds (+0.0194/+0.0159/+0.0159, all p<0.005); all six quality metrics sign-consistent 3/3; gate-C3 **now passes on 2/3 seeds** (s123 SF −0.39%/AG −0.60%; s3407 −0.21%/−0.36%) with s42 the AG borderline exception (−1.00%). See `GATE1_FREEZE.md` §3 for the verdict wording.
- **Falsification numbers are real and recorded**: SF −0.0190 / AG −0.0126 / QABF −0.0005 / MI +0.0213 (M3FD, 300 pairs, Holm); drift-AUC B0=S=0.7895, 0/5 resolutions. These stay in the paper as a negative result (protocol §1: "先改叙事,不改数据").
...
| No cross-scale drift improvement for the H1 (feature-space) arm (D1 stress axis) | eval_c256.log: drift-AUC B0=S=0.7895; 0/5 resolutions; p=9.45e-01. **Arm note (2026-09-13)**: H1-era F3 numbers — NOT in GATE1_FREEZE; if quoted in the manuscript they need their own freeze line, else omit from the final text (the CGA-arm F3 numbers in GATE1 §5 are the paper's cross-scale evidence). Distinct from CGA arm: B0 0.8088→CGA 0.7839, 4/5 resolutions | supported (own; freeze-status flag) |
...
| CGA improves downstream detection on high-conflict objects | eval_str_cga.log (full 300, 2026-08-25): high-conflict recall 0.4385→0.4579 (+0.0194), recovered=20 lost=3, p=4.88e-04; mAP50 0.7197→0.7282 (+0.0085) | **supported (significant)**; below stricter gates C1 (+0.02) / C2 (mAP50 +0.01) — report as such |
...
> Old rationale ("B/C/Δ are the mechanistic entry point into SSM dynamics") was invalidated by the H1 falsification and is retired. The rewrite below is what the paper carries; every clause traces to a number or a design fact.
